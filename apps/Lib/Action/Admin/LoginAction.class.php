    ...
    function lostpassword()
    {
        if(IS_POST){
            ...
            if ($r){
                ...
                $param['apiUser'] = 'grwy_test_S44sda'; # 使用api_user和api_key进行验证
                $param['apiKey'] = 'Qwp929ZaZR0A5LhU';
                $param['from'] = 'service@grwy.net'; # 发信人，用正确邮件地址替代
                $param['fromName'] = $config['site_name'];
                $param['to'] = $email;# 收件人地址, 用正确邮件地址替代, 多个地址用';'分隔
                $param['subject'] = '网站密码找回';
                $param['html'] = "尊敬的用户{$r['username']}，您好！<br><br><br>您可以通过点击以下链接重置帐户密码:<br><br><a href=\"{$url}\">{$url}</a><br><br>为保障您的帐号安全，请在24小时内点击该链接，您也可以将链接复制到浏览器地址栏访问。 若如果您并未尝试修改密码，请忽略本邮件，由此给您带来的不便请谅解。<br><br><br>本邮件由系统自动发出，请勿直接回复！";
                ...
                $return = file_get_contents('http://api.sendcloud.net/apiv2/mail/send', FILE_TEXT, $context);
                exit($return);
            } else {
                $this->error('邮箱不存在！');
            }
